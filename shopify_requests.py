...
def get_shopify_items(ignore_filter_conditions=False):
	...
	for page_idx in range(0, get_total_pages("products/count.json?", ignore_filter_conditions) or 1):
		shopify_products.extend(get_request('/admin/products.json')['products'])
...
def get_shopify_orders(ignore_filter_conditions=False):
	...
	for page_idx in range(0, get_total_pages("orders/count.json?status=any", ignore_filter_conditions) or 1):
		shopify_orders.extend(get_request('/admin/orders.json?status=any')['orders'])
...
def get_shopify_customers(ignore_filter_conditions=False):
	...
	for page_idx in range(0, get_total_pages("customers/count.json?", ignore_filter_conditions) or 1):
		shopify_customers.extend(get_request('/admin/customers.json')['customers'])
